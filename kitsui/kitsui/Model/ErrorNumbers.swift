...
//	Error numbers from 400 to 499 require waiting for remote operations; alert user and continue
	...
	let DBD_VItBItErr = 144		// Error deleting VerseItem for bridging
	let DBD_VItUBItErr = 145	// Error deleting VerseItem for unbridging

	let DBC_VItITiErr = 146		// Error creating VerseItem for IntroTitle
	let DBD_VItITiErr = 147		// Error deleting VerseItem for IntroTitle
	let DBC_VItIHdErr = 148		// Error creating VerseItem for IntroSubj
	let DBD_VItIHdErr = 149		// Error deleting VerseItem for IntroSubj
	let DBC_VItIPaErr = 150		// Error creating VerseItem for IntroPara
	let DBD_VItIPaErr = 151		// Error deleting VerseItem for IntroPara

	let DBC_BItErr = 152		// Error creating BridgeItem record
	let DBR_BItErr = 153		// Error reading BridgeItem record from database
	let DBD_BItErr = 154		// Error deleting BridgeItem record

	let DBU_ChaUSFMErr = 155	// Error updating USFM text in Chapter record

	let DBC_PopErr = 156		// Error creating a record during popover menu actions
	let DBR_PopErr = 157		// Error reading a record during popover menu actions
	let DBU_PopErr = 158		// Error updating a record during popover menu actions
	let DBD_PopErr = 159		// Error deleting a record during popover menu actions
